...
#' @export
#' @examples
#' scopic_model(counts.mat = counts.mat, type = c("genes", "cells"), n.topics = 3, n.starts = 5, seed = 12345, max.q = 0.05, min.coef = 1, min.spec = 0.1, min.sim = 0, model.return = FALSE)
#
...
scopic_model <- function(
	...
	if (model.return == TRUE) {
		...
		result.list["model"] <- list(result.lda)
		#
		} else {
			#
			return(result.stats)
